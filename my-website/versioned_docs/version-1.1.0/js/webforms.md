...
是给你用户来填，然后我来反应的一个东西
 ...
 理解表单基础    Understanding form basics
 文本框验证与交互Text box validation and interaction
 使用其他表单控件Working with other form controls
...
表单（Form）是在网页中用来收集用户输入数据的一种交互元素。它通常由各种输入字段、复选框、单选按钮、文本区域、提交按钮等组成。表单可以用来向服务器提交用户输入的数据，以便进一步处理或存储。
表单的主要用途是收集用户输入的信息，这些信息可以是用户个人信息、搜索查询、订阅服务、提交订单等等。通过表单，网站和应用程序可以与用户进行交互，并收集用户的反馈、需求或指示。
具体应用场景包括但不限于：

1. 用户注册和登录：网站可以使用表单来收集用户的注册信息，如用户名、密码、电子邮件地址等，并用于用户账户的创建和管理。
2. 搜索功能：搜索引擎和网站通常提供搜索表单，让用户输入关键字并提交查询，以搜索并显示相关内容。
3. 订阅和通知：网站提供订阅服务时，用户可以填写表单并订阅特定的内容或服务，如新闻、邮件列表等。
4. 在线购物：电子商务网站使用表单来收集用户的订单信息，如商品选择、送货地址、支付方式等，以完成购物流程。
5. 用户反馈：网站和应用程序可以通过表单收集用户的反馈意见、建议或投诉，以改进产品和服务。
总之，表单是网页中用来收集用户输入数据的重要工具，它为用户和网站之间的交互提供了便利和有效的方式。
...
// 例如，名为"form2"的表单可以通过 document.form2 来访问。不推荐使用这种方法，因为容易出错，而且这些属性将来可能会被浏览器删除。
...
```js
//Submitting Forms
//表单是通过用户点击提交按钮或图片按钮的方式提交的。 a submit button or an image button.提交按钮可以使用 type 属性为"submit"的<input>或<button>元素来定义，图片按钮可以使用 type 属性为"image"的<input>元素来定义。
...
// 提交表单
form.submit(); 
// 通过 submit()提交表单时，submit 事件不会触发。什么意思???如果使用 JavaScript 中的 submit() 方法来提交表单，通常情况下并不会触发表单的 submit 事件。这是因为 submit() 方法是直接通过编程方式提交表单的，而不是通过用户的交互动作触发的。因此，浏览器不会像用户手动提交表单那样触发 submit 事件。如果需要在使用 submit() 方法提交表单时执行一些特定的操作，开发者可以在调用 submit() 方法之后立即执行所需的操作，而不依赖于 submit 事件的触发。
...
// 禁用字段
field.disabled = true; 
// 改变字段的类型（不推荐，但对<input>来说是可能的）  为什么,什么意思???
field.type = "checkbox"; 
...
//  focus：在字段获得焦点时触发。
// blur 和 focus 事件会因为用户手动改变字段焦点或者调用 blur()或 focus()方法而触发。这两个事件对所有表单都会一视同仁。change 事件则不然，它会因控件不同而在不同时机触发。对于``<input>``和``<textarea>``元素，change 事件会在字段失去焦点，同时 value 自控件获得焦点后发生变化时触发。对于<select>元素，change 事件会在用户改变了选中项时触发，不需要控件失去焦点。focus 和 blur 事件通常用于以某种方式改变用户界面，以提供可见的提示或额外功能（例如在文本框下面显示下拉菜单）。change 事件通常用于验证用户在字段中输入的内容。比如，有的文本框可能只限于接收数值。focus 事件可以用来改变控件的背景颜色以便更清楚地表明当前字段获得了焦点。blur 事件可以用于去掉这个背景颜色。而 change 事件可以用于在用户输入了非数值时把背景改为红色。以下代码展示了上述操作：
...
textbox.addEventListener("focus", (event) => { 
 ...
 //为文本框添加一个 focus 事件监听器。当文本框获得焦点时，会触发此事件。在这个监听器中：
//获取事件的目标元素，即获得了焦点的文本框。
//检查文本框的背景颜色是否不是红色，如果不是红色则执行以下代码：
//将文本框的背景颜色设置为黄色。
}); 
...
textbox.addEventListener("blur", (event) => { 
 ...
 //为文本框添加一个 blur 事件监听器。当文本框失去焦点时，会触发此事件。在这个监听器中：
//获取事件的目标元素，即失去焦点的文本框。
//检查文本框的值是否包含非数字字符，如果包含非数字字符则将背景颜色设置为红色，否则将背景颜色清空。[^\d] 表示除了数字之外的任意字符。.test(target.value) 是正则表达式对象的方法，用于测试字符串是否匹配正则表达式。如果匹配成功，则返回 true，即目标元素的值包含非数字字符;否则返回 false。
}); 

textbox.addEventListener("change", (event) => { 
 let target = event.target; 
 target.style.backgroundColor = /[^\d]/.test(target.value) ? "red" : ""; 
 //为文本框添加一个 change 事件监听器。当文本框的值发生改变时，会触发此事件。在这个监听器中，执行的操作与 blur 事件监听器中的操作相同，即根据文本框的值是否包含非数字字符来设置背景颜色。
}); 

// 就是说当文本框获得焦点时，背景颜色变为黄色；当文本框失去焦点或其值发生改变时，检查其值是否包含非数字字符，如果是则将背景颜色设置为红色，否则将背景颜色清空。
// 这里的 onfocus 事件处理程序会把文本框的背景改为黄色，更清楚地表明它是当前活动字段。
// onblur 和 onchange 事件处理程序会在发现非数值字符时把背景改为红色。为测试非数值字符，这里使用了一个简单的正则表达式来检测文本框的 value。这个功能必须同时在 onblur 和 onchange 事件处理程序上实现，以确保无论文本框是否改变都能执行验证。
...
```js
//在 HTML 中有两种表示文本框的方式：单行使用<input>元素，多行使用<textarea>元素。这两个控件非常相似，大多数时候行为也一样。不过，它们也有非常重要的区别。默认情况下，<input>元素显示为文本框，省略 type 属性会以"text"作为默认值。然后可以通过size 属性指定文本框的宽度，这个宽度是以字符数来计量的。而 value 属性用于指定文本框的初始值，maxLength 属性用于指定文本框允许的最多字符数。因此要创建一个一次可显示 25 个字符，但最多允许显示 50 个字符的文本框，可以这样写：
...
// 同样与<input>元素不同的是，``<textarea>``不能在 HTML 中指定最大允许的字符数。除了标记中的不同，这两种类型的文本框都会在 value 属性中保存自己的内容。通过这个属性，
// 可以读取也可以设置文本模式的值，如下所示：
let textbox = document.forms[0].elements["textbox1"]; 
console.log(textbox.value); 
textbox.value = "Some new value"; 
// 应该使用 value 属性，而不是标准 DOM 方法读写文本框的值。比如，不要使用 setAttribute()设置<input>元素 value 属性的值，也不要尝试修改<textarea>元素的第一个子节点。对 value 属性的修改也不会总体现在 DOM 中，因此在处理文本框值的时候最好不要使用 DOM 方法。什么是DOM方法???
...
}); //是在用户选择文本框中的文本时，将被选择的文本打印到控制台中。

// 2. 取得选中文本Retrieving Selected Text
...
// 因为 substring()方法是基于字符串偏移量的，所以直接传入 selectionStart 和 selectionEnd就可以取得选中的文本。这个扩展在 IE9+、Firefox、Safari、Chrome 和 Opera 中都可以使用。IE8 及更早版本不支持这两个属性，因此需要使用其他方式。
...
function getSelectedText(textbox){ 
 ...
 //如果浏览器不支持 selectionStart 属性，那么这一行会检查 document.selection 是否存在。document.selection 是 IE 浏览器提供的 API，用于处理文本选择。   如果 document.selection 存在，说明当前浏览器是 IE 浏览器，它支持 createRange() 方法。这行代码将返回当前选中文本范围的文本内容，使用了 createRange().text。
 } 
} 

// 就是说这个函数根据当前浏览器的特性，获取文本框中被选中的文本，并返回该文本。
// 这个修改后的函数兼容在 IE 老版本中取得选中文本。注意 document.selection 是根本不需要textbox 参数的。
```

```js
// 3. 部分选中文本Partial Text Selection
...
range.select(); 
// 这一行将文本范围折叠到文本框的起始位置，即将选中范围设置为文本框的开头。
// range.moveStart("character", 0);: 这一行将选中范围的起始位置向前移动 0 个字符，实际上没有任何效果。
// range.moveEnd("character", textbox.value.length);: 这一行将选中范围的结束位置向后移动至文本框的末尾，选中了文本框中的所有文本。
// range.select();: 这一行触发了选中操作，将选中范围应用到文本框中，使得整个文本框中的文本都被选中了。
// 之后的两个操作是类似的，它们分别将选中范围设置为文本框中的前 3 个字符和第 4~6 个字符，然后触发选中操作，以此来选中相应的文本
...
textbox.addEventListener("keypress", (event) => { 
 ...
 //这是一个逻辑运算符，表示逻辑非，用于对表达式的结果取反。
// /\d/: 这是一个正则表达式，表示匹配任意数字字符的模式。\d是正则表达式中的元字符，表示数字字符。
// .test(): 这是正则表达式对象的方法，用于测试字符串是否匹配该正则表达式的模式。
// String.fromCharCode(event.charCode): 这是一个方法调用，用于将 ASCII 编码转换为字符。event.charCode 是事件对象的属性，表示触发键盘事件时按下的键的 ASCII 编码。
// 综合起来，这段代码的意思是：如果按下的键的字符不是数字，则执行条件语句内的代码块。
 { 
 event.preventDefault(); 
 } 
}); 
...
textbox.addEventListener("keypress", (event) => { 
 if (!/\d/.test(String.fromCharCode(event.charCode)) && 
 event.charCode > 9)
 //如果按下的键的字符不是数字且不是特殊键(ASCII 编码大于 9，即不是特殊键)，则执行条件语句内的代码块。  举个例子，假设你按下了字母键盘上的字母键 "A"，其 ASCII 编码为 65。在这种情况下，条件 event.charCode > 9 是真的（因为 65 大于 9），但是正则表达式 /\d/ 匹配不到字符 "A"，因为它不是一个数字字符。因此，整个条件语句会执行，以便处理不是数字字符的键。
 { 
 event.preventDefault(); 
 } 
}); 
...
```js
// 2. 处理剪贴板 Dealing with the Clipboard
...
// 同样，IE 支持"text"和"URL"，Safari 和 Chrome 则期待 MIME 类型。不过，与 getData()不同的是，Safari 和 Chrome 不认可"text"类型。只有在 IE8 及更早版本中调用 setData()才有效，其他浏览器会忽略对这个方法的调用。为抹平差异，可以使用以下跨浏览器的方法：
function getClipboardText(event){ 
 var clipboardData = (event.clipboardData || window.clipboardData); 
 return clipboardData.getData("text"); 
} 
//这个函数用于获取剪贴板中的文本内容。它首先检查 event.clipboardData 属性是否存在，如果存在则使用它来获取剪贴板数据；否则，它回退到使用 window.clipboardData 来获取数据。最后，它使用 getData("text") 方法来获取文本数据，并返回获取到的文本内容。
...
//这个函数用于设置剪贴板中的文本内容。它首先检查 event.clipboardData 是否存在，如果存在则使用它来设置剪贴板数据；否则，它回退到使用 window.clipboardData 来设置数据。最后，它使用 setData("text/plain", value) 方法来设置文本数据，并返回设置操作的结果。

// 两个函数的内部逻辑基本相同，只是一个用于获取剪贴板文本，另一个用于设置剪贴板文本。它们都首先检查浏览器是否支持 event.clipboardData 属性，如果支持则使用该属性来操作剪贴板数据；否则，它们回退到使用 window.clipboardData 来进行操作。
...
// $: 匹配字符串的结束位置。
// /: 正则表达式的结束标志。
// test: 这是正则表达式对象的一个方法，用于检测字符串是否匹配该正则表达式。
// text: 这是一个变量或者字符串，表示待匹配的文本内容。
// 因此，整个表达式的含义是：如果给定的文本 text 不全是由数字字符组成的话，条件判断结果为真。
 { 
 event.preventDefault(); 
 } 
}); 
...
```js
//Automatic Tab Forward就是说自动把内容给到下一个框
...
//定义了一个名为 tabForward 的函数，该函数用于处理文本框的键盘输入事件，并在需要时将焦点移动到下一个文本框。
// let target = event.target;：获取触发事件的目标元素，即用户当前正在输入的文本框。
// if (target.value.length == target.maxLength){ ... }：检查当前文本框的值的长度是否等于其最大长度。这是为了确定用户是否已经输入了足够的字符，以便将焦点移动到下一个文本框。
// let form = target.form;：获取包含当前文本框的表单元素。
// for (let i = 0, len = form.elements.length; i < len; i++) { ... }：遍历表单中的所有元素，以便找到当前文本框后面的下一个文本框。
// if (form.elements[i] == target) { ... }：检查当前遍历到的元素是否就是当前的文本框。
// if (form.elements[i+1]) { form.elements[i+1].focus(); }：如果找到了当前文本框后面的下一个文本框，则将焦点移动到该文本框。

 let inputIds = ["txtTel1", "txtTel2", "txtTel3"];
 //定义了一个数组 inputIds，包含了三个电话号码文本框的 ID。 
 for (let id of inputIds) { 
 let textbox = document.getElementById(id); 
 textbox.addEventListener("keyup", tabForward); 
 }  //历 inputIds 数组中的每个元素，即三个电话号码文本框的 ID。为每个文本框添加了一个 keyup 事件(是指当用户释放键盘上的键时触发的事件)监听器，当用户按键后会触发 tabForward 函数。
 let textbox1 = document.getElementById("txtTel1"); 
 let textbox2 = document.getElementById("txtTel2"); 
 let textbox3 = document.getElementById("txtTel3"); 
 //分别获取了三个电话号码文本框的元素。这些变量在后续代码中没有被使用，可能是未被删除的遗留代码。
``</script>``
// 这个 tabForward()函数是实现自动切换的关键。它通过比较用户输入文本的长度与 maxlength属性的值来检测输入是否达到了最大长度。如果两者相等（因为浏览器会强制最大字符数，所以不可能出现多的情况），那么就要通过循环表单中的元素集合找到当前文本框，并把焦点设置到下一个元素。这个函数接着给每一个文本框都指定了 onkeyup 事件处理程序。因为 keyup 事件会在每个新字符被插入到文本框中时触发，所以此时应该是检测文本框内容长度的最佳时机。在填写这个简单的表单时，用户不用按 Tab 键切换字段和提交表单。不过要注意，上面的代码只适用于之前既定的标记，没有考虑可能存在的隐藏字段。
...
//HTML5为浏览器新增了在提交表单前验证数据的能力。这些能力实现了基本的验证，即使JavaScript不可用或加载失败也没关系。这是因为浏览器自身会基于指定的规则进行验证，并在出错时显示适当的错误消息（无须 JavaScript）。这些能力只有支持 HTML5 这部分的浏览器才有，包括所有现代浏览器（除了 Safari）和 IE10+。验证会根据某些条件应用到表单字段。可以使用 HTML 标记指定对特定字段的约束，然后浏览器会根据这些约束自动执行表单验证。
```

```js
// 1. 必填字段Required Fields
// 第一个条件是给表单字段添加 required 属性，意思就是:必要一个值!,如下所示：
...
//首先，创建了一个新的 <input> 元素，并将其类型设置为 "email"。然后，通过检查这个输入元素的类型是否为 "email"，确定了当前浏览器是否支持 HTML5 中的 email 类型输入框。最终，这个信息被存储在 isEmailSupported 变量中，以便后续使用。
// 对于这两个新类型，除非应用了 required 属性，否则空字段是有效的。另外，指定一个特殊输入类型并不会阻止用户输入无效的值。新类型只是会应用一些默认验证。

// 3. 数值范围Numeric Ranges
...
//使用 JavaScript 中的 in 运算符检查在创建的 <input> 元素上是否存在 pattern 属性。如果存在，则返回 true，否则返回 false。整行代码的含义是：isPatternSupported 变量的值取决于当前浏览器是否支持 <input> 元素的 pattern 属性。如果支持，isPatternSupported 将被赋值为 true；如果不支持，则被赋值为 false。
```

```js
// 5. 检测有效性Checking Validity
...
//if (input.validity && !input.validity.valid) {: 这一行首先检查 input 元素的 validity 属性是否存在且有效。如果存在且有效，并且 valid 属性为 false，则进入条件语句块。
//if (input.validity.valueMissing) {: 在内部的条件语句中，首先检查 validity 对象的 valueMissing 属性。如果该属性为 true，则表示用户未输入任何值。
//console.log("Please specify a value."): 如果 valueMissing 为 true，则向控制台输出消息提示用户指定一个值。
//else if (input.validity.typeMismatch) {: 如果 valueMissing 不为 true，则检查 validity 对象的 typeMismatch 属性。如果该属性为 true，则表示用户输入的值与输入框的类型不匹配。
//console.log("Please enter an email address.");: 如果 typeMismatch 为 true，则向控制台输出消息提示用户输入一个电子邮件地址。
//else {: 如果以上两个条件都不满足，则进入这个 else 语句块，表示输入的值不符合预期的条件。
//onsole.log("Value is invalid.");: 在这个分支中，向控制台输出消息，指示用户输入的值无效。
//整段代码的内部逻辑是：首先检查输入框的有效性。如果输入无效，则根据不同的无效原因输出不同的提示消息。

// 6. 禁用验证Disabling Validation
...
//就是说名字叫btnNoValidate的元素的formNoValidate属性
```

## 选择框编程

SCRIPTING SELECT BOXES 就是可选内容的框框\下拉列表
...
// 如果选中这个选择框中的第一项，则字段的值就是"Sunnyvale, CA"。如果文本为"China"的项被选中，则字段的值是一个空字符串，因为该项的 value 属性是空字符串。如果选中最后一项，那么字段的值是"Australia"，因为该<option>元素没有指定 value 属性。  就是说选到啥是啥,有value就用value,没有就用框的文本内容.
...
// 最后强调一下，选择框的 change 事件与其他表单字段是不一样的。其他表单字段会在自己的值改变后触发 change 事件，然后字段失去焦点。而!!!选择框会在选中一项时立即触发 change 事件!!!。
...
//这行代码获取了 <select> 元素中当前选中的 <option> 元素。它通过 selectbox.selectedIndex 获取到当前选中项的索引，然后通过 selectbox.options[] 获取对应索引处的选项。
// 这样可以获取关于选项的所有信息，比如：
let selectedIndex = selectbox.selectedIndex; 
//这行代码将当前选中的选项的索引保存到变量 selectedIndex 中。这行代码与上一行的作用相同，只是将选中项的索引单独存储了起来。
let selectedOption = selectbox.options[selectedIndex]; 
//这行代码也是获取了当前选中的 <option> 元素，但是使用了之前存储的 selectedIndex 变量来获取。
console.log(`Selected index: ${selectedIndex}\n` + 
 `Selected text: ${selectedOption.text}\n` + 
 `Selected value: ${selectedOption.value}`); 
 //这是一条console.log()` 语句，它输出了关于选中项的一些信息。具体信息包括选中项的索引、文本内容和值。
//Selected index: ${selectedIndex}: 这部分输出了选中项的索引。
//Selected text: ${selectedOption.text}: 这部分输出了选中项的文本内容。
//Selected value: ${selectedOption.value}: 这部分输出了选中项的值
...
function getSelectedOptions(selectbox){ 
 ...
 return result; 
 //它遍历了 selectbox 元素的所有选项（<option> 元素），每次迭代将当前选项赋值给变量 option。
 //如果 option.selected 的值为 true，表示该选项被选中。
} 
...
for (let option of selectedOptions) { 
 ...
 //通过字符串模板将当前选项的索引、文本内容和值拼接到 message 变量中。
} 
console.log(message); 
...
// 第二种方式是使用选择框的 remove()方法。这个方法接收一个参数，即要移除选项的索引，比如：selectbox.remove(0); // 移除第一项
...
//将 selectbox1 中的第一个选项移动到 selectbox2 中
let selectbox1 = document.getElementById("selLocations1"); 
let selectbox2 = document.getElementById("selLocations2"); 
selectbox2.appendChild(selectbox1.options[0]); 
//selectbox1.options[0]: 获取了 selectbox1 的第一个选项（索引为 0 的选项）。selectbox2.appendChild(...): 将从 selectbox1 中获取的第一个选项作为子节点添加到 selectbox2 中。
...
selectbox.insertBefore(optionToMove, 
 ...
 //selectbox.insertBefore(...): 这是 DOM 操作的一种，用于在指定位置插入一个新的子节点。
//optionToMove: 要插入的节点，即要移动的选项。
//selectbox.options[optionToMove.index-1]: 插入位置的参考节点，这里通过 optionToMove.index-1 获取到了要移动选项的前一个选项节点。
//整段代码的内部逻辑是：将 selectbox 元素中的指定选项移动到其前一个选项之前。这可以用于重新排列选项顺序，将选项向上移动一位。
// 这个例子首先获得要移动选项的索引，然后将其插入之前位于它前面的选项之前，其中第二行代码适用于除第一个选项之外的所有选项。
...
// 遍历表单中的所有字段，根据字段的类型和状态进行不同的处理，将选中的选项和具有名称的字段序列化为键值对，并最终返回一个以 & 分隔的字符串形式的序列化结果。
function serialize(form) { 
 let parts = []; 
 let optValue; 
//这行代码定义了一个名为 serialize 的函数，它接受一个参数 form，表示一个表单元素。
//let parts = [];: 这行代码创建了一个空数组 parts，用于存储表单字段的序列化结果。
//let optValue;: 这行代码声明了一个变量 optValue，用于存储选项的值。

 for (let field of form.elements) { 

 switch(field.type) { 
 // 以下, 对于 <select> 元素，遍历其选项，将选中的选项序列化为键值对，并存储在 parts 数组中。
 case "select-one": 
 ...
 option.value : option.text); 
 //首先使用 option.attributes["value"] 获取到选项的 value 属性，然后检查该属性是否被指定。如果 specified 为 true，则表示 value 属性被指定了，那么 optValue 被赋值为 option.value；否则，使用 option.text 表示选项的文本内容，所以 optValue 被赋值为 option.text。
 } 
 parts.push(encodeURIComponent(field.name) + "=" +  encodeURIComponent(optValue)); 
 } 
 } 
 } 
 //这行代码检查当前字段是否具有名称。如果字段的名称长度大于 0，则进入条件语句块，表示这个字段需要被序列化。   这是一个 for...of 循环，遍历了当前字段（通常是一个 <select> 元素）的所有选项，每次迭代将当前选项赋值给变量 option。   在循环内部，检查当前选项是否被选中。    如果选中，则进入条件语句块，表示这个选项需要被序列化。这行代码检查当前选项是否具有 hasAttribute 方法。这里应该是一个错误，因为 hasAttribute 应该是一个方法，而不是一个属性。但是，这里意图是判断选项是否具有 value 属性。        这行代码根据选项是否具有 value 属性来确定要序列化的值。如果选项具有 value 属性，则使用 option.value，否则使用 option.text。         将当前字段的名称和选项的值序列化为一个键值对，并将其添加到 parts 数组中。键值对使用 = 分隔，然后使用 encodeURIComponent() 函数对键和值进行编码，以确保它们在 URL 中正确传输。        整段代码的内部逻辑是：对于具有名称的字段，遍历其所有选项，对于被选中的选项，根据选项是否具有 value 属性来确定序列化的值，并将其作为键值对添加到序列化结果数组中。
 break; 
 ...
 case "button": // 自定义按钮
 break; 
 //这些是表单元素的类型，但在序列化过程中不需要处理，因此在这些情况下直接跳过
...
```js
//在网页上编写富文本内容是 Web 应用开发中很常见的需求。富文本编辑也就是所谓的“所见即所得”（WYSIWYG，What You See Is What You Get）编辑。虽然没有规范定义，但源自 IE 的一套事实标准已经被 Opera、Safari、Chrome 和 Firefox 所支持。基本的技术就是在空白 HTML 文件中嵌入一个iframe。

// 通过 designMode 属性，可以将这个空白文档变成可以编辑的，实际编辑的则是<body>元素的 HTML。designMode 属性有两个可能的值："off"（默认值）和"on"。设置为"on"时，整个文档都会变成可以编辑的（显示插入光标），从而可以像使用文字处理程序一样编辑文本，通过键盘将文本标记为粗体、斜体，等等。
...
与富文本编辑器交互的主要方法是使用 document.execCommand()。这个方法在文档上执行既定的命令，可以实现大多数格式化任务。

document.execCommand()可以接收 3 个参数：要执行的命令、
表示浏览器是否为命令提供用户界面的布尔值和执行命令必需的值（如果不需要则为 null）。为跨浏览器兼容，第二个参数应该始终为 false，因为 Firefox 会在其为 true 时抛出错误。
...
| formatblock    | HTML 标签       | 将选中文本包含在指定的 HTML 标签中           |
| indent         | null            | 缩进文本                                   |
...
// 这些命令可以用于修改内嵌窗格（iframe）中富文本区域的外观，如下面的例子所示：\就是说(第一个是要执行的命令名字,第二个是false,第三个是命令需要的值)
// 在内嵌窗格中切换粗体文本样式
...
// 还有与命令相关的其他一些方法。第一个方法是 queryCommandEnabled()，就是说看此处此命令能不能行,此方法用于确定对当前选中文本或光标所在位置是否可以执行相关命令。它只接收一个参数，即要检查的命令名。如果可编辑区可以执行该命令就返回 true，否则返回 false。来看下面的例子：
let result = frames["richedit"].document.queryCommandEnabled("bold"); //检查在名为 "richedit" 的 frame 中，是否启用了粗体命令，并将结果存储在变量 result 中。
...
// 另一个方法 queryCommandState()用于确定相关命令是否应用到了当前文本选区,就是说看执行通畅不。例如，要确定当前选区的文本是否为粗体，可以这样：
let isBold = frames["richedit"].document.queryCommandState("bold"); //这里的 "bold" 表示粗体命令.   检查在名为 "richedit" 的 frame 中，当前光标处的文本是否为粗体，并将结果存储在变量 isBold 中。
...
使用标准的 DOM 技术，可以为选择框添加或移除选项，也可以将选项从一个选择框移动到另一个选择框，或者重排选项。
