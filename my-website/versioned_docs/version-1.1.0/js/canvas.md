...
使用 requestAnimationFrame
 ...
 理解``<canvas>``元素➤ Understanding the ``<canvas>`` element

 绘制简单 2D 图形➤ Drawing simple 2D graphics

 使用 WebGL 绘制 3D 图形➤ 3D drawing with WebGL
...
Similar to the other parts of the browser environment, ``<canvas>``is made up of a few API sets,and not all browsers support all API sets. There is a 2D context with basic drawing capabilitiesand a proposed 3D context called WebGL. The latest versions of the supporting browsers now support both the 2D context and WebGL.

就是说有围绕canvas元素 学api,2d,3d绘图

## 使用requestAnimationFrame

很长时间以来，计时器和定时执行都是 JavaScript 动画最先进的工具。虽然 CSS 过渡和动画方便了Web 开发者实现某些动画，但 JavaScript 动画领域多年来进展甚微。Firefox 4 率先在浏览器中为 JavaScript动画增加了一个名为 mozRequestAnimationFrame()方法的 API。这个方法会告诉浏览器要执行动画了，于是浏览器可以通过最优方式确定重绘的时序。自从出现之后，这个 API 被广泛采用，现在作为requestAnimationFrame()方法已经得到各大浏览器的支持。

### 早期定时动画

```js
//以前，在 JavaScript 中创建动画基本上就是使用 setInterval()来控制动画的执行。下面的例子展
示了使用 setInterval()的基本模式：
(function() { 
...
//这是一个立即执行函数表达式 (Immediately Invoked Function Expression, IIFE)，包裹了整个代码块。通过这种方式，函数内部的变量和函数定义不会污染全局作用域。
//function updateAnimations() { ... }: 这是定义了一个名为 updateAnimations 的函数，用于执行动画更新任务。函数内部调用了 doAnimation1() 和 doAnimation2() 来执行两个动画任务。
//setInterval(updateAnimations, 100): 这一行使用 setInterval 函数，每隔 100 毫秒（即每秒执行 10 次）就调用一次 updateAnimations 函数，从而执行动画更新任务。
//总体来说，这段代码的作用是每隔 100 毫秒执行一次 updateAnimations 函数，来更新动画效果。
// 作为一个小型动画库的标配，这个 updateAnimations()方法会周期性运行注册的动画任务，并反映出每个任务的变化（例如，同时更新滚动新闻和进度条）。如果没有动画需要更新，则这个方法既可以什么也不做，直接退出，也可以停止动画循环，等待其他需要更新的动画。这种定时动画的问题在于无法准确知晓循环之间的延时。定时间隔必须足够短，这样才能让不同的
// 动画类型都能平滑顺畅，但又要足够长，以便产生浏览器可以渲染出来的变化。一般计算机显示器的屏幕刷新率都是 60Hz，基本上意味着每秒需要重绘 60 次。大多数浏览器会限制重绘频率，使其不超出屏幕的刷新率，这是因为超过刷新率，用户也感知不到。
// 因此，实现平滑动画最佳的重绘间隔为 1000 毫秒/60，大约 17 毫秒。以这个速度重绘可以实现最平滑的动画，因为这已经是浏览器的极限了。如果同时运行多个动画，可能需要加以限流，以免 17 毫秒的重绘间隔过快，导致动画过早运行完。
// 虽然使用 setInterval()的定时动画比使用多个 setTimeout()实现循环效率更高，但也不是没有问题。无论 setInterval()还是 setTimeout()都是不能保证时间精度的。作为第二个参数的延时只能保证何时会把代码添加到浏览器的任务队列，不能保证添加到队列就会立即运行。如果队列前面还有其他任务，那么就要等这些任务执行完再执行。简单来讲，这里毫秒延时并不是说何时这些代码会执行，而只是说到时候会把回调加到任务队列。如果添加到队列后，主线程还被其他任务占用，比如正在
// 处理用户操作，那么回调就不会马上执行。
...
知道何时绘制下一帧是创造平滑动画的关键。直到几年前，都没有办法确切保证何时能让浏览器把下一帧绘制出来。随着``<canvas>``的流行和 HTML5 游戏的兴起，开发者发现 setInterval()和setTimeout()的不精确是个!!!大问题!!!。
...
```js
//Mozilla的 Robert O’Callahan一直在思考这个问题，并提出了一个独特的方案。他指出，浏览器知道 CSS过渡和动画应该什么时候开始，并据此计算出正确的时间间隔，到时间就去刷新用户界面。但对于 JavaScript动画，浏览器不知道动画什么时候开始。他给出的方案是创造一个名为 mozRequestAnimationFrame()的新方法，用以通知浏览器某些 JavaScript 代码要执行动画了。这样浏览器就可以在运行某些代码后进行适当的优化。目前所有浏览器都支持这个方法不带前缀的版本，即 requestAnimationFrame()。
...
requestAnimationFrame(updateProgress); 
// 这是定义了一个名为 updateProgress 的函数，用于更新进度条的状态。
//var div = document.getElementById("status");: 这一行通过 getElementById 方法获取了页面中 id 为 "status" 的元素，并将其赋值给变量 div，表示进度条的容器。
//div.style.width = (parseInt(div.style.width, 10) + 5) + "%";: 这一行用于增加进度条的宽度，首先通过 parseInt 方法将原始宽度字符串转换为整数，然后加上 5%，最后将新的宽度值赋值给 div.style.width，以更新进度条的宽度。将进度条的当前宽度字符串转换为整数。parseInt 函数的第一个参数是要转换的字符串，第二个参数是基数（即要解析的数字的进制，默认为十进制）。 这部分表示在当前进度条宽度的基础上增加 5，即进度条每次更新增加 5% 的宽度。 这部分将增加后的整数宽度转换为字符串形式的百分比。在最外层的括号内，整个表达式的结果先被计算，然后再将其转换为百分比形式，即添加 "%" 后缀。
//if (div.style.left != "100%") { ... }: 这一行检查进度条是否已经达到了 100% 的宽度。如果还没有达到，就继续执行下一帧的动画更新。
//requestAnimationFrame(updateProgress);: 这一行使用 requestAnimationFrame 方法来请求浏览器执行一次动画帧。在这里，它请求浏览器执行 updateProgress 函数，从而更新进度条的状态。这样可以使动画更新与浏览器的刷新频率同步，提高动画的流畅性和性能。
...
//实现了一种节流（throttling）机制，用于在滚动事件触发时执行昂贵的操作。
//定义了一个变量 enqueued，用于标记是否已经将昂贵的操作添加到请求动画帧队列中。
//定义了一个名为 expensiveOperation 的函数，用于执行昂贵的操作。在这里，它只是简单地打印当前时间戳到控制台，并将 enqueued 标记重置为 false。
//添加了一个滚动事件的监听器。每当页面滚动时，都会执行回调函数。
//在滚动事件的回调函数中，首先检查 enqueued 标志是否为 false。如果为 false，则表示还没有将昂贵的操作添加到请求动画帧队列中，可以继续执行。
//将 enqueued 标志设置为 true，表示已经将昂贵的操作添加到请求动画帧队列中。
//使用 requestAnimationFrame 方法请求浏览器在下一帧执行昂贵的操作。这样做可以确保操作不会在滚动事件频繁触发时立即执行，而是等待浏览器准备好执行动画帧时再执行。
// 因为重绘是非常频繁的操作，所以这还算不上真正的节流。更好的办法是配合使用一个计时器来限制操作执行的频率。这样，计时器可以限制实际的操作执行间隔，而requestAnimationFrame 控制在浏览器的哪个渲染周期中执行。下面的例子可以将回调限制为不超过 50 毫秒执行一次：
let enabled = true; 
function expensiveOperation() { 
 console.log('Invoked at', Date.now()); 
} 
...
//实现了一种防抖（debouncing）机制，用于在滚动事件触发时执行昂贵的操作，但确保在一段时间内只执行一次。
//定义了一个变量 enabled，用于控制是否可以执行昂贵的操作。初始状态为 true，表示允许执行操作。
//定义了一个名为 expensiveOperation 的函数，用于执行昂贵的操作。在这里，它只是简单地打印当前时间戳到控制台。
//添加了一个滚动事件的监听器。每当页面滚动时，都会执行回调函数。
//在滚动事件的回调函数中，首先检查 enabled 变量的值。如果为 true，则表示当前可以执行操作。
//将 enabled 变量设置为 false，表示防止在短时间内再次触发昂贵的操作。
//使用 requestAnimationFrame 方法请求浏览器在下一帧执行昂贵的操作。
//使用 setTimeout 方法设定一个计时器，在一定时间后（这里是 50 毫秒），将 enabled 变量重新设置为 true，表示允许再次触发昂贵的操作。
//这样，当页面滚动时，如果在一段时间内没有触发滚动事件，那么在下一次触发滚动事件时，会执行昂贵的操作。在操作执行后的一小段时间内，即使页面继续滚动，也不会再次执行昂贵的操作，从而提高了性能和用户体验。
```
...
// 获取页面中 id 为 "drawing" 的 <canvas> 元素，并将其赋值给变量 drawing。
//!!!检查浏览器是否支持 <canvas> 元素的上下文（context）!!!。如果支持，则进入条件语句块内部执行后续操作。
//使用 <canvas> 元素的 toDataURL() 方法将绘图数据转换为图像的数据 URI。这里指定转换为 PNG 格式的图像数据 URI，并将结果赋值给变量 imgURI。
//创建一个新的 <img> 元素，用于显示转换后的图像。
//将图像数据 URI 赋值给 <img> 元素的 src 属性，从而加载图像。
//将 <img> 元素添加到文档的 body 中，使其在页面上显示出来。
//通过这段代码，可以将 <canvas> 元素中的绘图内容以图像形式展示在页面上，方便用户查看和操作。
...
// 注意 描边宽度由 lineWidth 属性控制，它可以是任意整数值。类似地，lineCap 属性控制线条端点的形状［"butt"（平头）、"round"（出圆头）或"square"（出方头）］，而 lineJoin
// 属性控制线条交点的形状［"round"（圆转）、"bevel"（取平）或"miter"（出尖）］。

//clearRect就是说去矩形\使区域空白
...
2D 绘图上下文支持很多在画布上绘制路径的方法。通过路径可以创建复杂的形状和线条。要绘制路径，必须首先调用 beginPath()方法以表示要开始绘制新路径。然后，再调用下列方法来绘制路径。
 arc(x, y, radius, startAngle, endAngle, counterclockwise)：以坐标(x, y)为圆心，以 radius 为半径绘制一条弧线，起始角度为 startAngle，结束角度为 endAngle（都是
弧度）。最后一个参数 counterclockwise 表示是否逆时针计算起始角度和结束角度（默认为顺时针）。
...
//检查浏览器是否支持 <canvas> 元素的上下文（context）。如果支持，则进入条件语句块内部执行后续操作。
{ 
 let context = drawing.getContext("2d"); //使用 <canvas> 元素的 getContext() 方法获取 2D 渲染上下文，并将其赋值给变量 context。这里指定了 "2d" 参数，表示获取 2D 渲染上下文。

 // 创建路径，用于准备绘制图形。
 context.beginPath(); 

 // 绘制一个外圆。arc() 方法用于创建一条圆弧/曲线，参数依次为圆心的 x 坐标、y 坐标、半径、起始角度、结束角度以及是否逆时针绘制。这里绘制了一个半径为 99 的外圆。
 context.arc(100, 100, 99, 0, 2 * Math.PI, false); 

 // 移动当前绘制点到指定位置，用于创建路径的起始点。这里将当前绘制点移动到内圆的起始点。绘制一个内圆。与上一步的 arc() 方法类似，这里绘制了一个半径为 94 的内圆。
 context.moveTo(194, 100); 
 context.arc(100, 100, 94, 0, 2 * Math.PI, false); 

 // 再次移动当前绘制点到指定位置，用于绘制分针和时针的起始点。这里将当前绘制点移动到钟表的中心。  从当前绘制点绘制一条直线到指定位置，用于绘制分针。这里绘制了一条从钟表中心到钟表顶部的直线，表示分针。
 context.moveTo(100, 100); 
 context.lineTo(100, 15); 

 // 再次移动当前绘制点到指定位置，用于绘制时针的起始点。这里同样将当前绘制点移动到钟表的中心。  继续绘制一条直线到指定位置，用于绘制时针。这里绘制了一条从钟表中心到钟表左侧的直线，表示时针。
 context.moveTo(100, 100); 
...
//这个例子使用 arc()绘制了两个圆形，一个外圆和一个内圆，以构成表盘的边框。外圆半径 99 像素，原点为(100,100)，也就是画布的中心。要绘制完整的圆形，必须从 0 弧度绘制到 2π 弧度（使用数学常量 Math.PI）。而在绘制内圆之前，必须先把路径移动到内圆上的一点，以避免绘制出多余的线条。

//第二次调用 arc()时使用了稍小一些的半径，以呈现边框效果。然后，再组合运用 moveTo()和 lineTo()分别绘制分针和时针。最后一步是调用 stroke()，得到如图 18-4 所示的图像。路径是 2D 上下文的主要绘制机制，为绘制结果提供了很多控制。因为路径经常被使用，所以也有一个 isPointInPath()方法，接收 x 轴和 y 轴坐标作为参数。这个方法用于确定指定的点是否在路径上，可以在关闭路径前随时调用，比如：
...
let fontSize = 100; 
context.font = fontSize + "px Arial"; //设置绘图上下文的字体样式。这里将字体设置为 Arial，并使用 fontSize 变量指定字体大小。
while(context.measureText("Hello world!").width > 140) { 
 fontSize--; 
 context.font = fontSize + "px Arial"; 
 //循环执行以下操作，直到文本 "Hello world!" 的宽度小于或等于 140 像素为止。measureText() 方法用于测量指定文本在当前字体下的宽度。每次循环将 fontSize 减一，逐步减小字体大小。在每次循环中更新绘图上下文的字体大小。
} 
context.fillText("Hello world!", 10, 10); //在指定位置 (10, 10) 绘制文本 "Hello world!"。
context.fillText("Font size is " + fontSize + "px", 10, 50); //在指定位置 (10, 50) 绘制文本 "Font size is xxxpx"，其中 xxx 是当前的字体大小。这段文本用于显示动态调整后的字体大小。
...
context.save(); //将当前的绘图状态（包括当前的变换矩阵、剪切区域和其他属性）保存到绘图状态堆栈中。
...
context.restore();// 在调用 restore()之后，fillStyle 恢复为绿色，因此这一次绘制的矩形是绿色的。
context.fillRect(10, 10, 100, 200); // 在(10, 10)绘制绿色矩形

context.restore(); 
context.fillRect(0, 0, 100, 200); // 在(0, 0)绘制红色矩形
// 以上代码先将 fillStyle 设置为红色，然后调用 save()。接着，将 fillStyle 修改为绿色，坐标移动到(100, 100)，并再次调用 save()，保存设置。随后，将 fillStyle 属性设置为蓝色并绘制一个矩形。因为此时坐标被移动了，所以绘制矩形的坐标实际上是(100, 100)。而绘制矩形的坐标是(110, 110)，因为变换仍在起作用。再次调用 restore()之后，变换被移除，fillStyle 也恢复为红色。绘制最后一个矩形的坐标变成了(0, 0)。

// 注意，save()方法!!!只保存应用到绘图上下文的设置和变换!!!，不保存绘图上下文的内容。
```
...
// 以上代码获取了文本中的第一个图像，然后在画布上的坐标(10, 10)处将它绘制了出来。绘制出来的图像与原来的图像一样大。如果想改变所绘制图像的大小，可以再传入另外两个参数：目标宽度和目标高度。这里的缩放只影响绘制的图像，不影响上下文的变换矩阵。什么意思???比如下面的例子：
// context.drawImage(image, 50, 10, 20, 30); 
// 执行之后，图像会缩放到 20 像素宽、30 像素高。

// 还可以只把图像绘制到上下文中的一个区域。此时，需要给 drawImage()提供 9 个参数：要绘制的图像、源图像 x 坐标、源图像 y 坐标、源图像宽度、源图像高度、目标区域 x 坐标、目标区域 y 坐标、目标区域宽度和目标区域高度。这个重载后的 drawImage()方法可以实现最大限度的控制，比如：   就是说从前四个数的框子到后四个数的框子
// context.drawImage(image, 0, 10, 50, 50, 0, 100, 40, 60); 
// 最终，原始图像中只有一部分会绘制到画布上。这一部分从(0, 10)开始，50 像素宽、50 像素高。而绘制到画布上时，会从(0, 100)开始，变成 40 像素宽、60 像素高。
// 像这样可以实现如图 18-9 所示的有趣效果。

// 第一个参数除了可以是 HTML 的<img>元素，还可以是另一个<canvas>元素，这样就会把另一个画布的内容绘制到当前画布上。结合其他一些方法，drawImage()方法可以方便地实现常见的图像操作。操作的结果可以使用toDataURL()方法获取。不过有一种情况例外：如果绘制的图像来自其他域而非当前页面，则不能获取其数据。此时，调用 toDataURL()将抛出错误。比如，如果来自 www.example.com 的页面上绘制的是来自 www.wrox.com 的图像，则上下文就是“脏的”dirty，获取数据时会抛出错误。
```
...
context.fillRect(30, 30, 50, 50); //在画布上绘制一个渐变矩形，起始点坐标为 (30, 30)，宽度为 50，高度为 50。
// 为了让渐变覆盖整个矩形，而不只是其中一部分，两者的坐标必须搭配合适。以上代码将得到如图18-11 所示的结果。

// !!!如果矩形没有绘制到渐变的范围内，则只会显示部分渐变!!!。什么是渐变范围???比如：
context.fillStyle = gradient; 
...
context.fillRect(10, 10, 50, 50); 
// 绘制渐变矩形
context.fillStyle = gradient; //就是说填充方式来自变量gradient
context.fillRect(30, 30, 50, 50); 
...
//获取文档中的第一个图像元素，并将其赋值给变量 image。
// 使用图像创建一个填充模式。这里使用了图像 image 并指定了填充模式为 "repeat"，即在绘制形状时会重复平铺图像。
//将填充样式设置为前面创建的填充模式 pattern。
// 在画布上绘制一个矩形，并使用之前设置的填充模式填充。矩形的起始点坐标为 (10, 10)，宽度为 150，高度为 150。
//该代码段的内部逻辑是利用文档中的第一个图像元素创建了一个填充模式，然后使用该填充模式填充了一个矩形。填充模式的类型是重复平铺图像，因此矩形的填充效果是图像被重复铺满整个矩形区域。
...
if (drawing.getContext) { 
 ...
 for (i=0, len=data.length; i < len; i+=4) { 
  ...
  imageData.data = data; 
  context.putImageData(imageData, 0, 0); //将修改后的 ImageData 对象应用到画布上，从坐标 (0, 0) 开始绘制。
} 
// 这个例子首先在画布上绘制了一个图像，然后又取得了其图像数据。for 循环遍历了图像数据中的每个像素，注意每次循环都要给 i 加上 4。每次循环中取得红、绿、蓝的颜色值，计算出它们的平均值。
// 然后再把原来的值修改为这个平均值，实际上相当于过滤掉了颜色信息，只留下类似亮度的灰度信息。
// 之后将 data 数组重写回 imageData 对象。最后调用 putImageData()方法，把图像数据再绘制到画布上。结果就得到了原始图像的黑白版。
...
//其中，globalAlpha 属性是一个范围在 0~1 的值（包括 0 和 1），用于指定所有绘制内容的透明度，默认值为 0。如果所有后来的绘制都需要使用同样的透明度，那么可以将 globalAlpha 设置为适当的值，
//执行绘制，然后再把 globalAlpha 设置为 0。比如：
// 绘制红色矩形
context.fillStyle = "#ff0000"; 
context.fillRect(10, 10, 50, 50); 
// 修改全局透明度
...
WebGL 的全称是 "Web Graphics Library"，是一种用于在网页浏览器中渲染交互式 2D 和 3D 图形的 JavaScript API。这个名字的缩写 "WebGL" 指的是 "Web Graphics Library" 的首字母缩写。
WebGL 是画布的 3D 上下文。与其他 Web 技术不同，WebGL 不是 W3C 制定的标准，而是 Khronos Group 的标准。根据官网描述，“Khronos Group 是非营利性、会员资助的联盟，专注于多平台和设备下
并行计算、图形和动态媒体的无专利费开放标准”。Khronos Group 也制定了其他图形 API，包括作为浏览器中 WebGL 基础的 OpenGL ES 2.0。
OpenGL 这种 3D 图形语言很复杂，本书不会涉及过多相关概念。不过，要使用 WebGL 最好熟悉!!!OpenGL ES 2.0!!!，因为很多概念可以照搬过来。

本节假设读者了解 OpenGL ES 2.0 的基本概念，并简单介绍 OpenGL ES 2.0 在 WebGL 中实现的部分。
...
// 建议在!!!充分了解!!!这个选项的作用后再自行修改，因为这可能会影响性能。
...
// 这些上下文选项大部分适合开发高级功能。多数情况下，默认值就可以满足要求。如果调用 getContext()不能创建 WebGL 上下文，某些浏览器就会抛出错误。为此，最好把这个
...
//尝试获取 WebGL 上下文，并将其赋值给变量 gl
// 如果在 try 块中出现了异常，会跳到这里执行，异常对象会被赋值给变量 ex。
// 什么也不做是一个注释，表示在出现异常时不执行任何操作。
 }
 if (gl) { 
 // 使用 WebGL 
 } else { 
...
![alt text](canvas15.png)

The coordinate system within a viewport is different than the Inside of a viewport, the coordinates start with point (0, 0) in the center of the
viewport.
...
// 每次调用 gl.getError()方法会返回一个错误值。第一次调用之后，再调用 gl.getError()可能会返回另一个错误值。如果有多个错误，则可以重复这个过程，直到 gl.getError()返 回
...
```js
//Shaders
//着色器是 OpenGL 中的另一个概念。WebGL 中有两种着色器：顶点着色器和片段（或像素）着色器。vertex shaders and fragment shaders顶点着色器用于把 3D 顶点转换为可以渲染的 2D 点。片段着色器用于计算绘制一个像素的正确颜色。
//WebGL 着色器的独特之处在于，它们不是 JavaScript 实现的，而是使用一种与 C 或 JavaScript 完全不同的语言 GLSL（OpenGL Shading Language）写的。
...
// 一个 <script> 标签，它的 type 属性指定了脚本的类型，这里是顶点着色器。id 属性为 "vertexShader"，用于标识这段着色器代码。 
// attribute vec2 aVertexPosition; 
// 顶点着色器中的一个 attribute 变量，用于表示顶点的位置信息。vec2 表示一个包含两个浮点数的向量。
// void main() 
// 顶点着色器中的主函数，顶点着色器必须包含一个名为 main 的函数。
// { 
//  gl_Position = vec4(aVertexPosition, 0.0, 1.0); 
//  这行代码将顶点位置赋值给内置变量 gl_Position，vec4 表示一个包含四个浮点数的向量，前两个值来自 aVertexPosition，后两个值分别是 0.0 和 1.0。这将确定顶点在裁剪空间中的位置。
// } 
// </script> 
```
...
//注释:另一个 <script> 标签，它的 type 属性指定了脚本的类型，这里是片元着色器。id 属性为 "fragmentShader"，用于标识这段着色器代码。
//uniform vec4 uColor; 
//注释:片元着色器中的一个 uniform 变量，用于表示片元的颜色。vec4 表示一个包含四个浮点数的向量。
// void main() { 
//  gl_FragColor = uColor; 
// } 
//片元着色器中的主函数，片元着色器必须包含一个名为 main 的函数。 这行代码将 uniform 变量 uColor 的值赋给内置变量. 用于确定片元的最终颜色。
// </script> 
```
...
// 更复杂的 WebGL 应用可以动态加载着色器。重点在于要使用着色器，必须先拿到 GLSL 代码的字符串。有了 GLSL 字符串，下一步是创建 shader 对象。为此，需要调用 gl.createShader()方法，并传入想要创建的着色器类型（ gl.VERTEX_SHADER 或 gl.FRAGMENT_SHADER ）。然后，调用gl.shaderSource()方法把 GLSL 代码应用到着色器，再调用 gl.compileShader()编译着色器。下面是一个例子：
let vertexShader = gl.createShader(gl.VERTEX_SHADER); 
gl.shaderSource(vertexShader, vertexGlsl); 
gl.compileShader(vertexShader); 
//这行代码创建了一个顶点着色器对象，并将其赋值给变量 vertexShader。gl.createShader(type) 用于创建一个新的着色器对象，参数 type 指定了着色器的类型，这里是顶点着色器，使用了 gl.VERTEX_SHADER 常量。
//这行代码将顶点着色器的 GLSL（OpenGL Shading Language）源代码传递给顶点着色器对象。gl.shaderSource(shader, source) 用于指定着色器对象的 GLSL 源代码，参数 shader 是目标着色器对象，source 是包含 GLSL 源代码的字符串。
//这行代码编译了顶点着色器。gl.compileShader(shader) 用于编译指定的着色器对象，参数 shader 是目标着色器对象。

let fragmentShader = gl.createShader(gl.FRAGMENT_SHADER); 
gl.shaderSource(fragmentShader, fragmentGlsl); 
gl.compileShader(fragmentShader);
//创建了一个片元着色器对象，并将其赋值给变量 fragmentShader。与前面的顶点着色器类似，这里创建的是片元着色器，使用了 gl.FRAGMENT_SHADER 常量。
//这行代码将片元着色器的 GLSL 源代码传递给片元着色器对象，与顶点着色器类似。
//这行代码编译了片元着色器，与编译顶点着色器的步骤相同。
...
//创建了一个着色器程序对象，并将其赋值给变量 program。着色器程序是由顶点着色器和片元着色器组成的。
//这行代码将顶点着色器附加到着色器程序上。gl.attachShader(program, shader) 用于将指定的着色器对象附加到目标着色器程序上。
// 这行代码将片元着色器附加到着色器程序上，与附加顶点着色器的步骤相同。
//这行代码链接了着色器程序。链接着色器程序将顶点着色器和片元着色器连接在一起，形成一个可执行的着色器程序。
...
// 调试着色器和程序
// 与 WebGL 中的其他操作类似，着色器操作也可能失败，而且是静默失败。如果想知道发生了什么错误，则必须手工通过 WebGL 上下文获取关于着色器或程序的信息。对于着色器，可以调用 gl.getShaderParameter()方法取得编译之后的编译状态：
if (!gl.getShaderParameter(vertexShader, gl.COMPILE_STATUS)) { 
 alert(gl.getShaderInfoLog(vertexShader)); 
} 
// 这个例子检查了 vertexShader 编译的状态。如果着色器编译成功，则调用 gl.getShaderParameter()会返回 true。如果返回 false，则说明编译出错了。此时，可以使用 gl.getShaderInfoLog()并传入着色器取得错误。这个方法返回一个字符串消息，表示问题所在。gl.getShaderParameter()和gl.getShaderInfoLog()既可以用于顶点着色器，也可以用于片段着色器。着色器程序也可能失败，因此也有类似的方法。gl.getProgramParameter()用于检测状态。最常见的程序错误发生在链接阶段，为此可以使用以下代码来检查：
if (!gl.getProgramParameter(program, gl.LINK_STATUS)) { 
 alert(gl.getProgramInfoLog(program)); 
} 
// 与 gl.getShaderParameter()一样，gl.getProgramParameter()会在链接成功时返回 true，
// 失败时返回 false。当然也有一个 gl.getProgramInfoLog()方法，可以在程序失败时获取错误信息。
...
//  gl.LINE_LOOP：将数组作为一系列顶点，在这些顶点间绘制直线。从第一个顶点到第二个顶点绘制一条直线，再从第二个顶点到第三个顶点绘制一条直线，以此类推，直到绘制到最后一个
...
//  gl.TRIANGLES_STRIP：类似于 gl.TRIANGLES，区别在于前 3 个顶点之后的顶点会作为第三个顶点与其前面的两个顶点构成三角形。例如，如果数组中包含顶点 A、B、C、D，那么第一个
// 三角形使用 ABC，第二个三角形使用 BCD。
//  gl.TRIANGLES_FAN：类似于 gl.TRIANGLES，区别在于前 3 个顶点之后的顶点会作为第三个顶点与其前面的顶点和第一个顶点构成三角形。例如，如果数组中包含顶点 A、B、C、D，那么
...
// 除了使用 DOM 图片，这些步骤跟在 OpenGL 中创建纹理是一样的。最大的区别在于使用gl.pixelStorei()设置了像素存储格式。常量 gl.UNPACK_FLIP_Y_WEBGL 是 WebGL 独有的，在基
// 于 Web 加载图片时通常要使用。原因在于 GIF、JPEG 和 PNG 图片使用的坐标系统与 WebGL 内部的坐标系统不一样。如果不使用这个标志，图片就会倒过来。
...
//  如果这个类型是 gl.UNSIGNED_SHORT_5_6_5、gl.UNSIGNED_SHORT_4_4_4_4 或 gl.UNSIGNED_ SHORT_5_5_5_1，则定型数组必须是 Uint16Array。
...
```js
//WebGL1 代码几乎完全与 WebGL2 兼容。在使用 WebGL2 上下文时，唯一可能涉及修改代码以保证兼容性的就是扩展。在 WebGL2 中，很多扩展都变成了默认功能。例如，要在 WebGL1 中使用绘制缓冲区，需要先测试相应扩展后再使用：
