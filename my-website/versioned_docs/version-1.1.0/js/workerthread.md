...
// 此外，由于服务工作者线程几乎可以任意修改和重定向网络请求，以及加载静态资源，服务工作者线程API 只能在安全上下文（HTTPS）下使用。在非安全上下文（HTTP）中，navigator.serviceWorker是 undefined。为方便开发，浏览器豁免了通过 localhost 或 127.0.0.1 在本地加载的页面的安全上下文规则。
...
//  activate ：在服务工作者线程进入 激 活 或 已激活 状态时触发（在客户端可以通过ServiceWorkerRegistration.active 判断）。也可以在 self.onactive 属性上指定该事件的处理程序。
...
```js
//8. 服务工作者线程作用域限制Service Worker Scope Limitations
...
// 以下请求都会被拦截All of the following would be intercepted：
// fetch('/foo/fooScript.js'); 
// 以下请求都不会被拦截：
// fetch('/foo.js'); 
// fetch('/baz/bazScript.js');
...
的资源。可是，它对你想要做的事情做了很多假设，当应用程序没有完全遵循这些假设时，它就崩溃了。
...
本质上，服务工作者线程缓存机制是一个双层字典，其中顶级字典的条目映射到二级嵌套字典At its core, the service worker cache mechanism is a two-tier dictionary in which each entry in the top-level dictionary maps to a second nested dictionary.顶级字典是CacheStorage 对象，可以通过服务工作者线程全局作用域的 caches 属性访问。顶级字典中的每个值都是一个 Cache 对象，该对象也是个字典，是 Request 对象到 Response 对象的映射。
...
//将response1 存储在缓存 v1 中
caches.open('v1') 
.then((v1cache) => v1cache.put(request, response1)) 

//将response2 存储在缓存 v2 中
.then(() => caches.open('v2')) 
...
.then(console.log); // v1
//caches.match(request) 方法在所有缓存中查找第一个匹配的响应。在这个例子中，它首先检查缓存 v1 并找到匹配的响应 response1，然后返回它。因此，输出为 v1。
...
// 与 Map 类似，Cache 也有 delete()和 keys()方法。这些方法与 Map 上对应方法类似，但都基于期约。 如何使用 Cache API 来缓存请求和响应，查看缓存中的键，并从缓存中删除请求。在这个例子中，我们首先将一个请求-响应对存储在缓存中，然后查看缓存中的键，接着删除该请求，最后再次查看缓存中的键以验证删除操作是否成功:
const request1 = new Request('https://www.foo.com'); 
const response1 = new Response('fooResponse'); //创建请求和响应对象
...
.then((cache) => { 
 ...
 .then(console.log); // [] 
 //打开缓存 v1 并存储请求和响应
 //  caches.open('v1')：打开（或创建）一个名为 v1 的缓存。
 // cache.put(request1, response1)：在缓存 v1 中存储请求 request1 和响应 response1。
 // cache.keys()：获取缓存中的所有请求键。
 // console.log：输出缓存中的请求键。在存储请求和响应之后，应该会输出 [Request]。
 // cache.delete(request1)：从缓存中删除 request1 对应的条目。
 // cache.keys()：再次获取缓存中的所有请求键。
 // console.log：再次输出缓存中的请求键。在删除请求和响应之后，应该会输出 []。

 //这个示例，可以看出如何使用 Cache API 存储、查看和删除缓存中的请求和响应。
}); 
...
// 缓存是否命中取决于 URL 字符串和/或 Request 对象 URL 是否匹配。URL 字符串和 Request 对象是可互换的，因为匹配时会提取 Request 对象的 URL。下面的例子演示了这种互换性：如何使用 Cache API 来缓存多个请求和响应，并验证缓存内容。它展示了将不同类型的请求对象存储到缓存中，以及如何检索和输出缓存的响应内容:
const request1 = 'https://www.foo.com'; 
const request2 = new Request('https://www.bar.com');
const response1 = new Response('fooResponse'); 
const response2 = new Response('barResponse'); //创建请求和响应对象.request1 是一个字符串，表示一个 URL。request2 是一个 Request 对象，表示一个请求。response1 和 response2 是 Response 对象，分别包含 'fooResponse' 和 'barResponse' 作为响应内容。

caches.open('v1').then((cache) => { 
 cache.put(request1, response1) 
 .then(() => cache.put(request2, response2)) 
 //打开（或创建）一个名为 v1 的缓存
 //将 request1 和 response1 存储到缓存 v1 中。put 方法返回一个 Promise，表示操作完成
 //将 request2 和 response2 存储到缓存 v1 中

 .then(() => cache.match(new Request('https://www.foo.com'))) 
 .then((response) => response.text()) 
 .then(console.log) // fooResponse
 //匹配缓存中与 request1 对应的响应。
 //match 方法返回一个包含匹配响应的 Promise。
 //response.text()：将响应对象的内容转换为文本。
 //console.log：输出匹配的响应内容，应该是 'fooResponse'。

 .then(() => cache.match('https://www.bar.com')) 
 .then((response) => response.text()) 
 .then(console.log); // barResponse
}); 
 //匹配缓存中与 request2 对应的响应。将响应对象的内容转换为文本。输出匹配的响应内容，应该是 'barResponse'。

// Cache 对象使用 Request 和 Response 对象的 clone()方法创建副本，并把它们存储为键/值对。下面的例子演示了这一点，因为从缓存中取得的实例并不等于原始的键/值对： 将一个请求和响应存储到缓存中，并验证缓存中的请求对象和响应对象。然而，控制台输出的结果可能会与预期不同，即 false。这是因为 cache.put 方法在存储请求和响应时会创建新的请求和响应对象，存储的是它们的副本，因此进行严格相等比较时会返回 false:
const request1 = new Request('https://www.foo.com'); 
const response1 = new Response('fooResponse'); //创建一个指向 https://www.foo.com 的请求对象和一个包含 'fooResponse' 文本的响应对象

caches.open('v1') 
.then((cache) => { 
 cache.put(request1, response1) 
 //打开（或创建）一个名为 v1 的缓存.在缓存 v1 中存储请求 request1 和响应 response1。注意，这会存储它们的副本。

 .then(() => cache.keys()) 
 .then((keys) => console.log(keys[0] === request1)) // false  加粗  .获取缓存中的所有请求键。比较缓存中的第一个请求键与 request1 是否相等。由于缓存中存储的是请求的副本，因此严格相等比较会返回 false。

 .then(() => cache.match(request1)) 
 .then((response) => console.log(response === response1)); // false 加粗 .在缓存中匹配请求 request1 并获取对应的响应。比较匹配到的响应与 response1 是否相等。由于缓存中存储的是响应的副本，因此严格相等比较会返回 false。
}); 

//以下做一点修改.通过修改，我们可以验证缓存中存储的请求和响应内容是否正确，而不是直接比较对象的引用。
const request1 = new Request('https://www.foo.com'); 
const response1 = new Response('fooResponse'); 
...
caches.open('v1').then((cache) => { 
  cache.put(request1, response1)
    .then(() => cache.keys())
    .then((keys) => console.log(keys[0].url === request1.url)) // true
    .then(() => cache.match(request1))
    .then((response) => response.text())  //不同在于多这一句\比较的对象改动了一下
    .then((text) => console.log(text === 'fooResponse')); // true
});
...
//  ignoreMethod：设置为 true 时，在匹配 URL 时忽略请求查询的 HTTP 方法。比如下面的例子展示了 POST 请求匹配 GET 请求：  使用 Cache API 存储一个请求和响应，并通过 cache.match 方法匹配请求，同时展示了在请求方法不同的情况下匹配请求的结果，以及使用 ignoreMethod 选项来忽略请求方法进行匹配的结果:
const request1 = new Request('https://www.foo.com'); //创建一个指向 https://www.foo.com 的 GET 请求对象。
const response1 = new Response('fooResponse'); 
const postRequest1 = new Request('https://www.foo.com', 
 { method: 'POST' }); //创建一个指向foo.com的POST 请求对象。
caches.open('v1') 
.then((cache) => { 
 cache.put(request1, response1) 
 //打开（或创建）一个名为 v1 的缓存.在缓存 v1 中 放入 request1 和 response1。put 方法返回一个 Promise，表示操作完成。
 .then(() => cache.match(postRequest1)) 
 .then(console.log) // undefined. 尝试匹配 postRequest1。因为缓存中存储的是一个 GET 请求，而 postRequest1 是一个 POST 请求，所以匹配失败，返回 undefined。

 .then(() => cache.match(postRequest1, { ignoreMethod: true })) 
 .then(console.log); // Response {} .尝试匹配 postRequest1，但忽略请求方法。由于设置了 ignoreMethod: true，匹配成功，返回缓存中的响应对象。
}); 

//  ignoreVary：匹配的时候考虑 HTTP 的 Vary 头部，该头部指定哪个请求头部导致服务器响应不同的值。ignoreVary 设置为 true 时，在匹配 URL 时忽略 Vary 头部。 如何使用 Cache API 存储一个带有 Vary 头部的响应，并通过 cache.match 方法匹配请求，同时展示了在请求头部不匹配的情况下匹配请求的结果，以及使用 ignoreVary 选项来忽略 Vary 头部进行匹配的结果:
const request1 = new Request('https://www.foo.com'); 
const response1 = new Response('fooResponse', 
 { headers: {'Vary': 'Accept' }}); //创建一个包含 'fooResponse' 文本并带有 Vary: Accept 头部的响应对象。
const acceptRequest1 = new Request('https://www.foo.com', 
 { headers: { 'Accept': 'text/json' } }); //创建一个指向 https://www.foo.com 并带有 Accept: text/json 头部的请求对象。

caches.open('v1') 
.then((cache) => { 
 cache.put(request1, response1) 
 .then(() => cache.match(acceptRequest1)) 
 .then(console.log) // undefined.尝试匹配 acceptRequest1。由于 Vary: Accept 头部存在，缓存中存储的响应需要与请求的 Accept 头部匹配。因为缓存的请求没有 Accept 头部，而 acceptRequest1 有 Accept: text/json 头部，因此匹配失败，返回 undefined。
 .then(() => cache.match(acceptRequest1, { ignoreVary: true })) //尝试匹配 acceptRequest1，但忽略 Vary 头部。由于设置了 ignoreVary: true，忽略 Vary 头部，匹配成功，返回缓存中的响应对象。
...
理解服务工作者线程最终用途十分重要：让网页能够模拟原生应用程序Service workers should be understood through the lens of their overall intended purpose: to enable web pages to emulate native application behavior 。要像原生应用程序一样，服务工作者线程必须支持版本控制（versioning）。

从全局角度说，服务工作者线程的版本控制可以确保任何时候两个网页的操作都有一致性。该一致性可以表现为如下两种形式。

 代码一致性Code consistency。网页不是像原生应用程序那样基于一个二进制文件创建，而是由很多 HTML、CSS、JavaScript、图片、JSON，以及页面可能加载的任何类型的文件创建。网页经常会递增更新，即版本升级，以增加或修改行为。如果网页总共加载了 100 个文件，而加载的资源同时来自第 1版和第 2 版，那么就会导致完全无法预测，而且很可能出错。服务工作者线程为此提供了一种强制机制，确保来自同源的所有并存页面始终会使用来自相同版本的资源。

 数据一致性Data consistency。网页并非与外界隔绝的应用程序。它们会通过各种浏览器 API 如 LocalStorage或 IndexedDB 在本地读取并写入数据；也会向远程 API 发送请求并获取数据。这些获取和写入数据的格式在不同版本中可能也会变化。如果一个页面以第 1 版中的格式写入了数据，第二个页面以第 2 版中的格式读取该数据就会导致无法预测的结果甚至出错。服务工作者线程的资源一致性机制可以保证网页输入/输出行为对同源的所有并存网页都相同。

为确保一致性，服务工作者线程的生命周期不遗余力地避免出现有损一致性的现象。比如下面这些
可能。

 服务工作者线程提早失败Service workers fail early。在安装服务工作者线程时，任何预料之外的问题都可能阻止服务工作者线程成功安装。包括服务脚本加载失败、服务脚本中存在语法或运行时错误、无法通过importScripts()加载工作者线程依赖，甚至加载某个缓存资源失败。

 服务工作者线程激进更新Service workers aggressively update。浏览器再次加载服务脚本时（无论通过 register()手动加载还是基于页面重载），服务脚本或通过 importScripts()加载的依赖中哪怕有一个字节的差异，也会启动安装新版本的服务工作者线程。

 未激活服务工作者线程消极活动Inactive service workers passively activate。当页面上第一次调用 register()时，服务工作者线程会被安装，但不会被激活，并且在导航事件发生前不会控制页面。这应该是合理的：可以认为当前页面已加载了资源，因此服务工作者线程不应该被激活，否则就会加载不一致的资源。

 活动的服务工作者线程粘连Active service workers are sticky。只要至少有一个客户端与关联到活动的服务工作者线程，浏览器就会在该源的所有页面中使用它。浏览器可以安装新服务工作者线程实例以替代这个活动的实例，但浏览器在与活动实例关联的客户端为 0（或强制更新服务工作者线程）之前不会切换到新工作者线程。这个服务工作者线程逐出策略能够防止两个客户端同时运行两个不同版本的服务工作者线程。
...
```js
navigator.serviceWorker.register('./serviceWorker.js')
  //注册位于 ./serviceWorker.js 文件中的 Service Worker。这个方法返回一个 Promise，该 Promise 在注册成功时解析为一个 ServiceWorkerRegistration 对象。
.then((registration) => {
 registration.installing.onstatechange = ({ target: { state } }) => {
 console.log('state changed to', state);
 };
});
// 监听 Service Worker 安装状态的变化：
// .then((registration) => { ... })：当注册成功时，执行回调函数，回调函数的参数是 registration 对象。
// registration.installing：这是一个 ServiceWorker 对象，代表正在安装的 Service Worker 实例。
// registration.installing 可能是 null，只有在 Service Worker 正在安装时才会有值。
// registration.installing.onstatechange：给 installing 对象的 onstatechange 事件赋值一个事件处理函数，用于监听状态变化。
// 当 Service Worker 的状态发生变化时，会触发 statechange 事件。
// ({ target: { state } }) => { ... }：事件处理函数的参数是一个事件对象，通过解构赋值获取事件对象中的 target.state 属性。
// console.log('state changed to', state)：在控制台输出 Service Worker 状态的变化。
```
...
// 所有这些任务全部成功，则 register()返回的期约会解决为一个ServiceWorkerRegistration对象。新创建的服务工作者线程实例进入到安装中状态。
```
...
// 在客户端，这个阶段可以通过检查 ServiceWorkerRegistration.installing 是否被设置为ServiceWorker 实例：
navigator.serviceWorker.register('./serviceWorker.js') 
.then((registration) => { 
...
// 关联的 ServiceWorkerRegistration 对象也会在服务工作者线程到达该状态时触发 updatefound事件：
navigator.serviceWorker.register('./serviceWorker.js') 
.then((registration) => { 
...
// waitUntil()方法，该方法接收一个期约参数，会将状态过渡延迟到这个期约解决。例如，下面的例子可以延迟 5 秒再将状态过渡到已安装状态：
self.oninstall = (installEvent) => { 
...
// 更接近实际的例子是通过 Cache.addAll()缓存一组资源之后再过渡：如何在 Service Worker 的安装事件中预缓存资源。通过监听 install 事件，您可以在 Service Worker 安装期间将一些资源添加到缓存中:
const CACHE_KEY = 'v1'; 
self.oninstall = (installEvent) => { 
...
//定义缓存的键
//监听 Service Worker 的 install 事件，并定义事件处理函数.installEvent 是安装事件对象。如果没有错误发生或者没有拒绝，服务工作者线程就会前进到已安装状态。
//在安装事件中预缓存资源：使用 waitUntil 方法确保在 Service Worker 安装过程中完成指定的异步操作。如果操作未完成或失败，Service Worker 的安装步骤将会失败。打开一个名为 CACHE_KEY（即 'v1'）的缓存。如果该缓存不存在，将会创建一个新的缓存。在缓存打开后，使用 cache.addAll 方法将指定的资源列表添加到缓存中。addAll 方法接受一个包含多个请求的数组，并将每个请求的响应添加到缓存中。在这里，我们将 foo.js、bar.html 和 baz.css 添加到缓存中。
```

```js
//3. 已安装状态The Installed State
// 已安装状态也称为等待中（waiting）状态，意思是服务工作者线程此时没有别的事件要做，只是准备在得到许可的时候去控制客户端。如果没有活动的服务工作者线程，则新安装的服务工作者线程会跳到这个状态，并直接进入激活中状态，因为没有必要再等了。

// 在客户端，这个阶段可以通过检查 ServiceWorkerRegistration.waiting 是否被设置为一个ServiceWorker 实例来确定：
navigator.serviceWorker.register('./serviceWorker.js') 
.then((registration) => { 
...
const CACHE_KEY = 'v3'; //定义一个常量 CACHE_KEY，用于指定新的缓存名称
self.oninstall = (activateEvent) => { 
 caches.keys() 
 //获取所有缓存的键，返回一个包含所有缓存名称的 Promise。
 .then((keys) => keys.filter((key) => key != CACHE_KEY)) 
 //过滤掉与当前缓存键 CACHE_KEY 不匹配的旧缓存键，保留需要删除的旧缓存键。 .filter(...) 是 JavaScript 数组的方法，用于创建一个新数组，其中包含通过指定函数测试的所有元素;参数是一个回调函数，它会对数组中的每个元素执行。
 .then((oldKeys) => oldKeys.forEach((oldKey) => caches.delete(oldKey));
 // 对每个旧缓存键调用 caches.delete(oldKey) 以删除对应的缓存。
}; 
//监听 Service Worker 的 install 事件，并定义事件处理函数。 activateEvent 是激活事件对象。注意，这里实际应该是监听 activate 事件，而不是 install 事件。install 事件通常用于缓存资源，而 activate 事件通常用于清理旧缓存。
...
//以下是调整监听的事件为 activate，因为这是更合适的事件来处理旧缓存的清理
const CACHE_KEY = 'v3'; 
self.onactivate = (activateEvent) => {
  activateEvent.waitUntil(
    caches.keys()
      .then((keys) => keys.filter((key) => key !== CACHE_KEY))
      .then((oldKeys) => Promise.all(oldKeys.map((oldKey) => caches.delete(oldKey))))
  );
};
...
// 更可靠的确定服务工作者线程处于已激活状态一种方式是检查 ServiceWorkerRegistration 的controller 属性。该属性会返回激活的 ServiceWorker 实例，即控制页面的实例：
navigator.serviceWorker.register('./serviceWorker.js') 
.then((registration) => { 
...
```js
//6. 已失效状态The Redundant State
// 已失效状态表示服务工作者线程已被宣布死亡。不会再有事件发送给它，浏览器随时可能销毁它并回收它的资源。

// 7. 更新服务工作者线程Updating a Service Worker
...
//  发生了 fetch()或 push()等功能性事件， 且 至少 24 小时内没有发生更新检查。
...
这样就意味着服务工作者线程不应该依赖工作者线程的全局状态。服务工作者线程中的绝大多数代码应该在事件处理程序中定义。当然，服务工作者线程的版本作为全局常量是个显而易见的例外。服务脚本执行的次数变化很大，高度依赖浏览器状态，因此服务脚本的行为应该是幂等的The number of times a service worker script will execute is wildly variable and highly dependent on browser state, and therefore the service worker script’s behavior should be idempotent。
...
浏览器仍在渐进地支持这个选项，因此强烈推荐读者同时使用 updateViaCache 和 CacheControl头部指定客户端的缓存行为。
...
// 实现上述操作的基本代码如下。如何在 Service Worker 的安装和激活过程中进行缓存填充，并立即激活新的 Service Worker，以接管现有的客户端,确保在新 Service Worker 安装后立即生效:
const CACHE_KEY = 'v1'; 
self.oninstall = (installEvent) => { 
   //监听 Service Worker 的 install 事件，并定义事件处理函数。
 // 填充缓存，然后强制服务工作者线程进入已激活状态,这样会触发 activate 事件
 installEvent.waitUntil( 
   //使用 waitUntil 方法确保在 Service Worker 安装过程中完成指定的异步操作。如果操作未完成或失败，Service Worker 的安装步骤将会失败。
 caches.open(CACHE_KEY) 
 //打开一个名为 CACHE_KEY（即 'v1'）的缓存。如果该缓存不存在，将会创建一个新的缓存。
 .then((cache) => cache.addAll([ 
 'foo.css', 
 'bar.js', 
 //在缓存打开后，使用 cache.addAll 方法将指定的资源列表添加到缓存中。在这里，我们将 foo.css 和 bar.js 添加到缓存中。
 ])) 
 .then(() => self.skipWaiting()) 
 //在资源缓存完成后，调用 self.skipWaiting() 方法，强制正在安装的 Service Worker 跳过等待状态，立即进入激活状态。这会触发 activate 事件。
 ); 
}; 

// 强制服务工作者线程接管客户端,这会在每个客户端触发 controllerchange 事件
//激活事件处理函数：监听 Service Worker 的 activate 事件，并定义事件处理函数。\立即接管所有客户端，这会触发每个客户端的 controllerchange 事件。
self.onactivate = (activateEvent) => clients.claim();

// 浏览器会在每次导航事件中检查新服务脚本，但有时候这样也太不够了。ServiceWorkerRegistration对象为此提供了一个 update()方法，可以用来告诉浏览器去重新获取服务脚本，与现有的比较，然后必要时安装更新的服务工作者线程。可以这样来实现：注册一个 Service Worker 并定期检查其更新。具体来说，它每约 17 分钟检查一次更新版本，以确保 Service Worker 是最新的:
navigator.serviceWorker.register('./serviceWorker.js') 
.then((registration) => { 
 // 每 17 分钟左右检查一个更新版本:使用 setInterval 方法设置一个定时器，每隔 1E6 毫秒（即 1,000,000 毫秒或 1000 秒，约 16.67 分钟）执行一次更新检查。
...
//监听消息事件：self.onmessage：为 Service Worker 设置一个消息事件处理程序，当 Service Worker 接收到来自客户端的消息时，这个处理程序会被调用。
//({data, source})：从事件对象中解构出 data 和 source 属性。data：包含来自客户端的消息内容。source：表示消息的发送者，可以用来回复消息。
//处理消息并发送回复：在控制台输出接收到的消息内容。使用 source.postMessage 方法向客户端发送一条消息 'bar'。

main.js 
navigator.serviceWorker.onmessage = ({data}) => { 
 console.log('client heard:', data); 
 //监听 Service Worker 的消息事件：navigator.serviceWorker.onmessage：\为 navigator.serviceWorker 设置一个消息事件处理程序，当客户端从 Service Worker 接收到消息时，这个处理程序会被调用。\({data})：从事件对象中解构出 data 属性。data：包含来自 Service Worker 的消息内容。
}; 
navigator.serviceWorker.register('./serviceWorker.js') 
.then((registration) => { 
   //注册 Service Worker：navigator.serviceWorker.register('./serviceWorker.js')：注册一个新的 Service Worker，文件路径为 ./serviceWorker.js。\.then((registration) => { ... })：注册成功后，返回一个 registration 对象，表示已注册的 Service Worker。
 if (registration.active) { 
 registration.active.postMessage('foo'); 
 //向 Service Worker 发送消息：if (registration.active) { registration.active.postMessage('foo'); }：检查 registration.active 是否存在，即 Service Worker 是否处于激活状态。如果存在，使用 registration.active.postMessage('foo') 向 Service Worker 发送消息 'foo'。
 } 
}); 
//当主线程向激活的 Service Worker 发送消息 'foo' 时，Service Worker 会接收到并在控制台输出 'service worker heard: foo'。\然后，Service Worker 使用 source.postMessage('bar') 回复一条消息 'bar' 给客户端。\最后，客户端接收到来自 Service Worker 的消息 'bar'，并在控制台输出 'client heard: bar'。
...
navigator.serviceWorker.register('./serviceWorker.js') 
//navigator.serviceWorker.register('./serviceWorker.js')：使用 navigator.serviceWorker.register 方法注册一个新的 Service Worker，文件路径为 ./serviceWorker.js。    .then(() => { ... })：注册成功后，执行 then 方法内的回调函数。
.then(() => { 
 if (navigator.serviceWorker.controller) { 
 navigator.serviceWorker.controller.postMessage('foo'); 
 //检查是否由 Service Worker 控制,就是问是由serviceworker控制吗?:if (navigator.serviceWorker.controller)：检查 navigator.serviceWorker.controller 是否存在。navigator.serviceWorker.controller 表示当前页面的控制器 Service Worker，如果存在，说明当前页面已被 Service Worker 控制。navigator.serviceWorker.controller.postMessage('foo')：如果 controller 存在，向控制器 Service Worker 发送消息 'foo'。
 } 
}); 
// service worker heard: foo 
// client heard: bar 
...
// 如果服务工作者线程需要!!!率先发送消息!!!，可以像下面这样获得客户端的引用：如何在 Service Worker 的激活事件和客户端之间进行双向通信。通过监听和发送消息，主线程和 Service Worker 可以实现消息传递和交互:
ServiceWorker.js 
self.onmessage = ({data}) => { 
 console.log('service worker heard:', data); 
}; 
self.onactivate = () => { 
 ...
 //处理激活事件：self.onactivate：为 Service Worker 设置一个激活事件处理程序。当 Service Worker 激活时，这个处理程序会被调用。    self.clients.matchAll({includeUncontrolled: true})：使用 clients.matchAll 方法获取所有客户端，未受控的客户端也included。  .then((clientMatches) => clientMatches[0].postMessage('foo'));：从匹配的客户端中选取第一个，并向其发送消息 'foo'。
}; 
...
navigator.serviceWorker.onmessage = ({data, source}) => { 
 console.log('client heard:', data); 
 source.postMessage('bar'); 
//监听 Service Worker 的消息事件
}; 
navigator.serviceWorker.register('./serviceWorker.js') //注册 Service Worker
//当主线程注册 Service Worker 并且 Service Worker 激活时，Service Worker 会向客户端发送消息 'foo'。   客户端接收到消息 'foo' 后，会在控制台输出 'client heard: foo'，并向 Service Worker 发送消息 'bar'。    Service Worker 接收到消息 'bar' 后，会在控制台输出 'service worker heard: bar'。
// client heard: foo 
...
服务工作者线程最重要的一个特性就是拦截网络请求。服务工作者线程作用域中的网络请求会注册为 fetch 事件。这种拦截能力不限于 fetch()方法发送的请求，也能拦截对 JavaScript、CSS、图片和
...
FetchEvent 继承自 ExtendableEvent。让服务工作者线程能够决定如何处理 fetch 事件的方法是 event.respondWith()。该方法接收期约，该期约会解决为一个 Response 对象。当然，该 Response对象实际上来自哪里完全由服务工作者线程决定。可以来自网络，来自缓存，或者动态创建。下面几节将介绍几种网络/缓存策略，可以在服务工作者线程中使用。

```js
//1. 从网络返回Return from Network
// 这个策略就是简单地转发 fetch 事件。那些绝对需要发送到服务器的请求例如 POST 请求就适合该策略。可以像下面实现这一策略：
self.onfetch = (fetchEvent) => { 
 //监听 fetch 事件:self.onfetch：为 Service Worker 设置一个 fetch 事件处理程序。当任何网络请求（例如加载页面的资源、发起的 API 请求等）发生时，这个处理程序会被调用。fetchEvent：代表 fetch 事件的对象，包含有关该网络请求的详细信息。

 fetchEvent.respondWith(fetch(fetchEvent.request)); 
 //使用 respondWith 方法处理请求:fetchEvent.respondWith：这个方法用来提供一个响应给浏览器，以替换默认的网络请求响应。就是说拿这个来respond.    fetch(fetchEvent.request)：使用 fetch 方法发起网络请求，参数是 fetchEvent.request，即原始的请求对象。这会导致 Service Worker 继续执行默认的网络请求行为。
}; 
...
```js
//3. 从网络返回，缓存作后备Return from Network with Cache Fallback
...
self.onfetch = (fetchEvent) => { 
 ...
 //fetchEvent.respondWith：这个方法用来提供一个响应给浏览器，以替换默认的网络请求响应。
 //fetch(fetchEvent.request)：使用 fetch 方法发起网络请求，参数是 fetchEvent.request，即原始的请求对象。
 //.catch(() => caches.match(fetchEvent.request))：如果网络请求失败（例如网络断开），则在缓存中查找匹配的请求并返回缓存的响应。
}; 
 //这段代码使得所有被拦截的 fetch 请求首先尝试进行网络请求。如果网络请求成功，返回网络响应。如果网络请求失败（例如因为没有网络连接），则尝试在缓存中查找匹配的请求并返回缓存的响应
```

```js
//4. 从缓存返回，网络作后备Return from Cache with Network Fallback
// 这个策略优先考虑响应速度，但仍会在没有缓存的情况下发送网络请求。这是大多数渐进式 Web应用程序（PWA，Progressive Web Application）采取的首选策略：如何在 Service Worker 中处理 fetch 事件，优先返回缓存中的响应，如果缓存中没有匹配的响应则继续进行网络请求:
self.onfetch = (fetchEvent) => { 
 fetchEvent.respondWith( 
 caches.match(fetchEvent.request) 
 .then((response) => response || fetch(fetchEvent.request)) 
 ); 
 //caches.match(fetchEvent.request)：在缓存中查找与请求匹配的响应。
 //.then((response) => response || fetch(fetchEvent.request))：如果缓存中有匹配的响应，则返回缓存响应；如果没有，则进行网络请求。  就是说如果 response 是 null 或 undefined（即缓存中没有找到响应），它将继续评估 fetch(fetchEvent.request) 并返回其值。
};
```

```js
//5. 通用后备Generic Fallback
...
self.onfetch = (fetchEvent) => { 
 ...
 //尝试在缓存中查找与请求匹配的响应
 .then((response) => response || fetch(fetchEvent.request)) 
 //then 方法中的箭头函数处理缓存查找的结果。如果缓存中有响应，则返回该响应；否则发起网络请求。
 .catch(() => caches.match('/fallback.html')) 
 //如果 fetch 请求失败（例如，网络不可用），catch 方法处理错误。caches.match('/fallback.html')：返回缓存中的后备响应，例如一个备用的 HTML 页面。
 ); 
}; 
...
```js
//1. 显示通知Displaying Notifications
...
```js
//2. 处理通知事件Handling Notification Events
// 通过 ServiceWorkerRegistration 对象创建的通知会向服务工作者线程发送 notificationclick和 notificationclose 事件。假设前面例子中的服务脚本定义了如下事件处理程序：
...
// 在这个例子中，与通知的两种交互操作都在服务工作者线程中注册了处理程序。这里的 notification事件对象暴露了 notification 属性，其中包含着生成该事件 Notification 对象。这些处理程序可以决定交互操作之后的响应方式。
...
```js
//3. 订阅推送事件Subscribing to Push Events
// 对于发送给服务工作者线程的推送消息，必须通过服务工作者线程的 PushManager 来订阅。这样服务工作者线程就可以在 push 事件处理程序中处理推送消息。

// 下面展示了使用 ServiceWorkerRegistration.pushManager 订阅推送消息的例子：如何在注册 Service Worker 后订阅推送通知:
navigator.serviceWorker.register('./serviceWorker.js') //注册一个 Service Worker，./serviceWorker.js' 作为其文件路径。
 .then((registration) => { 
   //注册后执行 then 方法中的回调函数。registration 是一个 ServiceWorkerRegistration 对象，表示注册的 Service Worker。
 registration.pushManager.subscribe({ 
 applicationServerKey: key, // 来自服务器的公钥,registration.pushManager.subscribe 方法订阅推送通知,返回一个 PushSubscription 对象，表示当前用户的推送订阅信息。
 userVisibleOnly: true 
 //设置为true，表示此订阅仅用于显示可见的通知。强制性的选项，表示每个推送必须显示通知给用户。
 }); 
}); 

// 另外，服务工作者线程也可以使用全局的 registration 属性自己订阅：将推送订阅逻辑放在 onactivate 事件处理程序中，确保在服务工作者激活时立即订阅推送通知。以下展示了如何在服务工作者激活后自动订阅推送通知:
self.onactivate = () => { 
   //设置一个事件处理程序来处理 activate 事件。该事件处理程序将在服务工作者激活时执行。
 self.registration.pushManager.subscribe({ 
   // self.registration.pushManager.subscribe 方法订阅推送通知,返回一个 PushSubscription 对象，表示当前用户的推送订阅信息。
 applicationServerKey: key, // 来自服务器的公钥,applicationServerKey 是从服务器获取的公钥，用于推送通知的加密。key 变量应该包含您的应用服务器的公钥（通常是一个 Base64 编码的字符串）。
 userVisibleOnly: true 
 }); 
};
```
...
```js
//4. 处理推送事件Handling Notification Events
// 订阅之后，服务工作者线程会在每次服务器推送消息时收到 push 事件。这时候它可以这样来处理：如何在 push 事件中处理推送数据，并将其打印到控制台：
self.onpush = (pushEvent) => { 
   //设置一个事件处理程序来处理 push 事件。处理程序将在服务工作者接收到推送消息时执行。pushEvent 是一个 PushEvent 对象，包含推送消息的数据。
 console.log('Service worker was pushed data:', pushEvent.data.text()); 
 // console.log 将推送的数据打印到控制台。pushEvent.data.text() 方法用于提取推送消息的文本内容。
}; 
// 为实现真正的推送通知，这个处理程序只需要通过注册对象创建一个通知即可。不过，完善的推送通知需要创建它的服务工作者线程保持活动足够长时间，以便处理后续的交互事件。
// 要实现这一点，push 事件继承了 ExtendableEvent。可以把 showNotification()返回的期约传给waitUntil()，这样就会让服务工作者线程一直活动到通知的期约解决。

// 下面展示了实现上述逻辑的简单框架：在主线程和服务工作者中处理推送通知的订阅、显示和点击事件：
main.js 
navigator.serviceWorker.register('./serviceWorker.js') 
.then((registration) => { 
   //注册成功后，执行回调函数，传入 registration 对象。
 Notification.requestPermission() 
   // 请求显示通知的授权
 .then((status) => { 
   //(授权完成),执行回调函数，传入授权状态 status。
    if (status === 'granted') { 
   // 如果用户授予了显示通知的权限
 registration.pushManager.subscribe({
   //订阅推送通知。 
 applicationServerKey: key, // 传入服务器提供的公钥 key，用于推送通知的加密。
 userVisibleOnly: true 
 //指定此订阅仅用于显示可见的通知。
 }); 
 } 
...
//监听 push 事件，收到推送消息后显示通知。
self.onpush = (pushEvent) => { 
    //设置一个事件处理程序来处理 push 事件。
 pushEvent.waitUntil( 
    // 保持服务工作者线程活动到通知期约解决
...
//监听 notificationclick 事件，用户点击通知时打开特定的页面。
self.onnotificationclick = ({notification}) => { 
   //设置一个事件处理程序来处理 notificationclick 事件。
 clients.openWindow('https://example.com/clicked-notification');
   //打开一个新的浏览器窗口，URL 为 'https://example.com/clicked-notification'。 
}; 

//实现一个完整的推送通知处理流程，从订阅到显示和响应用户点击。
```
